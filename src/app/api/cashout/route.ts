...
export async function GET(req: Request) {
  try {
    ...
    const cashout = await getAllCashout(
...
export async function POST(req: Request) {
  try {
    ...
    const inputValidation = CreateCashoutInputValidation.safeParse(body);
    ...
    } as CreateCashoutData);

    return success_response(
      cashout,
      "Cashout successful, we will process your request immediately.",
      200
    );
  } catch (err) {
    return error_response((err as any)?.message, 400);
